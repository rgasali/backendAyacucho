...
function mostrarLista() {
  ...
  for (let i = 0; i < lista.length; i++) {
    tablaId += `<p>${lista[i].identificador}<br><p>`;
    tablaCancion += `<p>${lista[i].cancion}<br><p>`;
    tablaDuracion += `<p>${lista[i].duracion}<br><p>`;
    tablaInterprete += `<p>${lista[i].interprete}<br><p>`;
    tablaGenero += `<p>${lista[i].genero}<br><p>`;
    tablaFecha += `<p>${lista[i].fecha}<br><p>`;
  }
...
function calcularMayorDuracion(lista) {
  console.log(
    `El tema mas largo es  ${lista[lista.length - 1].cancion} que dura ${
      lista[lista.length - 1].duracion
    }`,
  );
}
//autor con mas temas
